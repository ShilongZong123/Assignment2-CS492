...
void make_info(struct task_info* info) {
...
static int cache_add(pid_t pid, pid_t tgid)
{
        struct object *obj;
...
void scull_cleanup_module(void)
{
	...
	printk("Linked List to be deleted: ");
		...
		if (i != 1) {
			printk(" -> Task %d: PID: %d, TID: %d", i, p->pid, p->tgid);
		} else {
			printk("Task %d: PID: %d, TID: %d", i, p->pid, p->tgid);
		}
...
int scull_init_module(void)
{
	...
	printk(KERN_NOTICE"init scull ok\n");
	return 0; /* succeed */
